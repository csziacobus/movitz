...
(defun declare-form-p (form &optional (declare-symbol 'muerte.cl::declare))
  (and (consp form) (eq declare-symbol (car form))))
...
(defun parse-body (forms &optional (declare-symbol 'muerte.cl::declare))
  ...
  (let (docstring)
    (loop for current = (car forms)
          if (declare-form-p current declare-symbol)
            append (rest (pop forms)) into declarations
          else if (and (stringp current) (cdr forms))
